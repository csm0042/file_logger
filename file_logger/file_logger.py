#!/usr/bin/python3
""" file_logger.py: Log file setup helper functions
"""
...
def setup_log_files(name):
    """ Function to decide where to store log files (folder in root) and create that folder
    if necessary """
...
def setup_log_handlers(name, debug_logfile, info_logfile):
    """ Function to configure root logger with three handlers, one to stream info and up messages,
    plus two additional file handlers for debug and info messages """
